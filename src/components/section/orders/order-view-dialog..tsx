...
import {useGetAllOrdersQuery, useOrderStatusMutation, useUpdateOrderMutation} from "@/features/api/orderSlice.ts";
...
export function OrderViewDialog({order, open, onOpenChange}: OrderViewDialogProps) {
    ...
    const {refetch} = useGetAllOrdersQuery({
        page: 1, // Replace with actual page
        limit: 10, // Replace with actual limit
    });
    ...
    const handelAddNewProduct = (value: any) => {
        ...
        const productExists = products.some((item: any) => item.product._id === product._id);
        if (productExists) {
    ...
    const handleSave = async () => {
        ...
        try {
            const response = await updateOrder({orderId: order?._id, updates: updatedData}).unwrap();
            if (response) {
                            ...
                            >
                                ...
                                <SelectContent>
                                    {['pending', 'delivered', 'cancelled'].map((status) => (
                                        <SelectItem key={status} defaultValue={'pending'} value={status}>
                            ...
                            >
                                ...
                                <SelectContent>
                                    {allProducts
                                        ...
                                        .concat(
                                            searchQuery.length > 0 && !allProducts.some((product) =>
                                                product?.name?.toLowerCase().includes(searchQuery.toLowerCase())
                                            ) ? (
                        ...
                        {order?.editable ? (
                                <div className="space-y-2">
                                    {products.map((product: any, index: number) => (
                                        ...
                                        >
                                            <span
                                                className="text-sm">{product?.product?.name || `Product ${index + 1}`}</span>
